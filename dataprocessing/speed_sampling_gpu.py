    ...
    for batch_id in range(math.floor(th_batch.shape[0] / batch_size) + 1):
        ...
        for tg in tg_batch:
            if iter>2 and iter < len(rob_corner_list):
                ...
                combine = torch.cat(
                    (nonsep.unsqueeze(1), where_coll.unsqueeze(1)), dim=1
                )
...
def arm_obstacle_distance(th_batch, chain, out_path_, triangles_obs):
    ...
    for batch_id in range(math.floor(th_batch.shape[0]/batch_size)+1):
        ...
        tg_batch = chain.forward_kinematics(
            th_batch[
                batch_id
                * batch_size : min((batch_id + 1) * batch_size, th_batch.shape[0]),
                :,
            ],
            end_only=False,
        )
        ...
        for tg in tg_batch:
            if iter>1:
                ...
                v, f = igl.read_triangle_mesh(
                    out_path_ + "/meshes/collision/" + tg + ".obj"
                )
        ...
        distance, closest_points, closest_faces, closest_bcs = bvh(
            triangles_obs, query_points
        )
        torch.cuda.synchronize()
        distance = torch.sqrt(distance).squeeze()
        distance = torch.reshape(distance, (-1, point_size))
...
def arm_rand_sample_bound_points(numsamples, dim, 
    ...
    X_list = []
    Y_list = []
    X_list, Y_list = arm_append_list(
        X_list, Y_list, chain, out_path_, t_obs, numsamples, dim, offset, margin
    )
...
def point_append_list(X_list, Y_list, triangles_obs, numsamples, dim, offset, margin):
    OutsideSize = numsamples + 2
    WholeSize = 0
    while OutsideSize > 0:
...
def point_rand_sample_bound_points(numsamples, dim, v_obs, f_obs, offset, margin):
    numsamples = int(numsamples)
    v_obs = torch.tensor(v_obs, dtype=torch.float32, device='cuda')
    f_obs = torch.tensor(f_obs, dtype=torch.long, device='cuda')
    t_obs = v_obs[f_obs].unsqueeze(dim=0)
    X_list = []
    Y_list = []
    X_list, Y_list = point_append_list(
        X_list, Y_list, t_obs, numsamples, dim, offset, margin
    )
    X = torch.cat(X_list,0)[:numsamples]
    Y = torch.cat(Y_list, 0)[:numsamples]
    sampled_points = X.detach().cpu().numpy()
